# -*- coding: utf-8 -*-
"""
Created on Tue May 25 13:22:57 2021

@author: https://github.com/brentp/combat.py/blob/master/combat.py
"""
...
def combat(data, batch, model=None, numerical_covariates=None):
    ...
    #data = data.T   #they seem to have it flipped by default
...
def combat_transform(data, batch, gamma_star, delta_star, stand_mean, var_pooled,model=None, numerical_covariates=None):
    ...
    for j, batch_idxs in enumerate(batch_info):
        print(j)
        #print(j,batch_idxs,"idxs\n")
        #print(bayesdata,bayesdata.shape,"bayesdata\n")
        dsq = np.sqrt(delta_star[j,:])
        dsq = dsq.reshape((len(dsq), 1))
        #print("done reshaping",dsq)
        denom =  np.dot(dsq, np.ones((1, n_batches[j]))) #divide by sqrt delta_star
        #print("done denom",denom)
        numer = np.array(bayesdata[batch_idxs] - np.dot(batch_design.loc[batch_idxs], gamma_star).T) #subtract gamma_star
        #print("done numer",numer.shape)
        ds = numer / denom
        #print("done with division step",ds)
        bids = list(batch_idxs)
        #print(bids)
        bayesdata.loc[:,bids] = ds
        #print("done assigning ds")
