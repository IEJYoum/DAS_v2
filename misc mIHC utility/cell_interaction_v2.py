...
def extract_mask_regions(mask: np.ndarray, score: np.ndarray,
                         ...
                         connectivity: int) -> tuple[np.ndarray, list[dict], dict[str, int]]:
    """Label a fixed final mask, filter components, and rank by peak score."""
    ...
    candidates, rejected_small, rejected_large = [], 0, 0
    for raw_label in range(1, n_labels):
        ...
        candidates.append({"raw_label": raw_label,
    ...
    return raw_labels, candidates, {"mask_pixels": int(mask.sum()),
...
def save_composite_png(score: np.ndarray,
    ...
    ax.set_title("Final interaction score + accepted regions", fontsize=11)
    ax.axis("off")
    fig.savefig(path, dpi=100, bbox_inches="tight")
    plt.close(fig)
...
def process_image(image_desc: dict,
    ...
    print(f"  Final score threshold: {FINAL_SCORE_THRESHOLD:g}")
    ...
    # --- Final gated score mask -> components -> ranked annotations ---
    region_mask = create_region_mask(score, marker_gate_mask, FINAL_SCORE_THRESHOLD,
                                     close_radius_px)
    raw_labels, candidates, region_selection = extract_mask_regions(
    ...
    print(f"  Final mask: {region_selection['mask_pixels']} pixels; "
          ...
          f"{region_selection['eligible_components']} pass area filters "
          f"({region_selection['rejected_small']} too small, "
          f"{region_selection['rejected_large']} too large)")
    ...
    for rank, candidate in enumerate(candidates, start=1):
        ...
        acc_entry = {'label': label_id, 'seed_y': candidate['seed_y'],
                     ...
                     'threshold': FINAL_SCORE_THRESHOLD, 'raster_area_um2': raster_area_um2,
                     'n_pixels': candidate['n_pixels'],
                     'seed_y_fullres': candidate['seed_y'] * effective_downsample,
                     'seed_x_fullres': candidate['seed_x'] * effective_downsample,
                     'seed_y_um': candidate['seed_y'] * effective_pixel_size,
                     'seed_x_um': candidate['seed_x'] * effective_pixel_size}
